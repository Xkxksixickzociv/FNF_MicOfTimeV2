...
function onCreatePost()
	...
	precacheImage('ratings/MOT(old)/shit');
	precacheImage('ratings/MOT(old)/bad');
	precacheImage('ratings/MOT(old)/good');
	precacheImage('ratings/MOT(old)/sick');
	precacheImage('ratings/MOT(old)/num');
end	
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if not isSustainNote and resetHideHud and getProperty('combo') then
		...
		makeLuaSprite('lastRating', 'ratings/MOT(old)/' .. rating, screenWidth * 0.35 - -42 + downscrolloffset + 680 , screenHeight / 2 - 120 - downscrolloffset * 3 + 200 );
		...
		scaleObject('lastRating', 0.9, 0.9);
		...
		if combo >= 0 then
			for i=2,0,-1 do
				...
				makeLuaSprite(tag, 'ratings/MOT(old)/num' .. math.floor(combo / 10 ^ i % 10) , screenWidth * 0.35 - -105 - i * 43 + downscrolloffset + 680, screenHeight / 2 + -0 - downscrolloffset * 3 + 200 );
